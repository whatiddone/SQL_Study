...
1. 지표 고민: 어떤 문제를 해결하기 위해(문제 정의) 데이터가 필요한가?
2. 지표 구체화(**지표 정의**): 추상적이지 않고 구체적인 지표 명시(분자, 분모 표시)
3. 지표 탐색: 유사한 문제를 해결한 케이스가 있는지 확인
- 있는 경우 -> 해당 쿼리 리뷰(내부 데이터, 구글, GPT...)
- 없는 경우 -> 쿼리 작성
4. 쿼리 작성: 데이터가 있는 테이블 찾기
  - 테이블 1개-> 바로 활용
  - 테이블 2개 이상-> 테이블 간 연결 방법 고민(JOIN)
...
### 쿼리 작성 템플릿
```
# 쿼리 작성 목표, 확인할 지표: (지표의 정의 작성)
# 쿼리 계산 방법: (집계를 하는가, 컬럼만 쓰면 되는가, 어떻게 계산을 해야 하는가)
# 데이터의 기간: 
# 사용할 테이블: 
# Join KEY: (연결할 수 있는 부분)
# 데이터 특징: 
...
### 생산성 도구: 템플릿 쉽게 사용하기
- 생산성 도구: Espanso
  - 특정 단어를 입력하면 원하는 문장(정의된 템플릿)으로 변경
